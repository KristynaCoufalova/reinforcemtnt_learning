...
def main(env: MultiArmedBandits, args: argparse.Namespace) -> float:
    # Initialize the estimates for all bandits, to `args.initial`.
    estimates = np.ones(args.bandits) * args.initial
    
    # Keep track of the number of times each action was chosen
    counts = np.zeros(args.bandits)
    
    rewards = 0
    for step in range(args.episode_length):
        # Select either a greedy action (if `env.greedy(args.epsilon)` is True)
        # or uniformly random action (otherwise).
        if env.greedy(args.epsilon):
            # Greedy action - choose the one with the highest estimate
            action = np.argmax(estimates)
        else:
            # Random action - choose uniformly
        ...
        # Update parameters, either using averaging (when `args.alpha` == 0)
        # or by an update with a learning rate of `args.alpha`.
        counts[action] += 1
        
        if args.alpha == 0:
            # Update using averaging
            estimates[action] += (reward - estimates[action]) / counts[action]
        else:
            # Update using fixed learning rate
